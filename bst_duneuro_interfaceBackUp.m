...
if ~isfield(cfg,'isotrop'); cfg.BstDuneuroVersion = 2; end                 % 1 previous with separate files, 2the new version combined eeg and meg and binary + txt output,   
...
disp(['duneruo >>0 - Changing path from ' cfg.currentPath ' to ' (fullfile(cfg.pathOfTempOutPut))])
...
disp(['duneruo >>1 - Writing the head file  to ' ((cfg.pathOfTempOutPut))])
...
disp(['duneruo >>2 - Writing the dipole file  to ' ((cfg.pathOfTempOutPut))])
...
disp(['duneruo >>3 - Writing the electrode  file  to ' ((cfg.pathOfTempOutPut))])
...
disp(['duneruo >>4 - Writing the conductivity/tensor file  to ' ((cfg.pathOfTempOutPut))])
...
disp(['duneruo >>5 - Writing the duneuro configuration file  to ' ((cfg.pathOfTempOutPut))])
cfg = bst_prepare_minifile(cfg);

%% 6- Run the duneuro
if cfg.runFromBst == 1; bst_progress('text', 'Duneuro:  run fem computation ... '); end
disp(['duneruo >>6 - Run duneuro binaries from ' (fullfile(cfg.pathOfTempOutPut))])
% define the command line
cfg = bst_set_duneuro_cmd(cfg);

% run Duneuro
disp(['duneruo >>6 - Run duneuro binaries from ' (fullfile(cfg.pathOfTempOutPut)) '... done' ])
cfg = bst_run_duneuro_cmd(cfg);

%% 7- Read the lead field matrix
disp(['duneruo >>7 - Read the leadfield from ' (fullfile(cfg.pathOfTempOutPut))])
cfg = bst_read_duneuro_leadfield(cfg);

%% substract the mean or not from the electrode
disp('duneuro >>8 - Postprocess the  leadfield ')
...
disp(['duneruo >>9 - Going back to  ' cfg.currentPath ]);
cd(cfg.currentPath)
