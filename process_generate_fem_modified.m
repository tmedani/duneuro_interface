function varargout = process_generate_fem_modified( varargin )
...
switch lower(OPTIONS.Method)
    ...
    case 'simnibs'
        ...
        if contains(OPTIONS.MeshType, 'tetrahedral')
        ...
        if contains(OPTIONS.MeshType, 'hexahedral')
            ...
            %                [sMri, vox2ras]  = in_mri_nii(maskfilename);    bMri = in_mri(maskfilename)
            [sMri, vox2ras]  = in_mri(maskfilename);    %bMri = in_mri(maskfilename)
            % Re-write to the fieldtrip format
            %                mri_segmented0 = bstMri2ftMri(sMri, vox2ras);               ftpMri = out_fieldtrip_mri(sMri);
            mri_segmented = out_fieldtrip_mri(sMri);            %   ftpMri = out_fieldtrip_mri(sMri);
            ...
            % create to data base
            FemHexa.Comment = [subjid 'hexa mesh'];
            FemHexa.Vertices = mesh.pos;
            FemHexa.Elements = mesh.hex;
            FemHexa.Tissue = mesh.tissue;
            FemHexa.TissueLabels =  {'white'  'gray'  'csf'  'skull'  'scalp'};
            FemHexa.History =  [];
            
            % Save to database
            FemFile = file_unique(bst_fullfile(bst_fileparts(T1File), sprintf('tess_fem_%s_%s_%dV.mat', OPTIONS.Method,'hexa', length(FemHexa.Vertices))));
            bst_save(FemFile, FemHexa, 'v7');
            db_add_surface(iSubject, FemFile, FemHexa.Comment);
...
elseif strcmp(OPTIONS.MeshType, 'hexahedral')
