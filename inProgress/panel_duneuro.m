...
% PANEL_OPENMEEG: Options for OpenMEEG Fem (GUI).
% 
...
function [bstPanelNew, panelName] = CreatePanel(sProcess, sFiles)  %#ok<DEFNU>  
    ...
    jPanelLayers = gui_river([4,4], [3,15,10,10], 'FEM Layers & conductivities');
        ...
        for i = 1:nLayers
            ...
            if isfield(OPTIONS, 'FemFiles') && ~isempty(OPTIONS.FemFiles)
                fields = whos('-file', OPTIONS.FemFiles{i});
